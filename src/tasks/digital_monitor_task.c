...
static void digital_monitor_task(void *arg)
{
  ...
  ESP_ERROR_CHECK(adc_inputs_init());
  ESP_ERROR_CHECK(ld_monitor_init(&ld_cfg));
  ESP_ERROR_CHECK(tec_monitor_init(&tec_cfg));

  digital_snapshot_t last = {0};

  TickType_t last_wake = xTaskGetTickCount();
  for (;;)
  {
    vTaskDelayUntil(&last_wake, pdMS_TO_TICKS(DIGITAL_MONITOR_PERIOD_MS));
    const TickType_t now = xTaskGetTickCount();
    digital_snapshot_t cur = {.valid = true};

    pwr_monitor_status_t pwr = {0};
    if (pwr_monitor_sample(&pwr) == ESP_OK)
    {
      system_status_update_pwr(pwr.tec_ready, pwr.ld_ready, now);
      cur.tec_ready = pwr.tec_ready;
      cur.ld_ready = pwr.ld_ready;
    }
    ...
    if (ld_monitor_sample_digital(&ld) == ESP_OK)
    {
      ...
      cur.ld_pwr_good = ld.pwr_good;
      cur.ld_loop_good = ld.loop_good;
    }
    ...
    if (tec_monitor_sample_digital(&tec) == ESP_OK)
    {
      ...
      cur.tec_pwr_good = tec.pwr_good;
      cur.tec_temp_good = tec.temp_good;
    }

    if (!last.valid || memcmp(&cur, &last, sizeof(cur)) != 0)
    {
      ESP_LOGI(TAG,
               "inputs pwr{tec_ready=%d ld_ready=%d} ld{pgood=%d lpgd=%d} tec{pgood=%d tempgd=%d}",
               cur.tec_ready ? 1 : 0, cur.ld_ready ? 1 : 0, cur.ld_pwr_good ? 1 : 0, cur.ld_loop_good ? 1 : 0,
               cur.tec_pwr_good ? 1 : 0, cur.tec_temp_good ? 1 : 0);
      last = cur;
    }
  }
}
